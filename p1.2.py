...
PARAMS = {
    ...
    "end_time": "2017-09-07 14:00:00",  # 回测结束时间
...
def initialize(context):
    ...
    context.user_data.cbrate = 0.055
    ...
    context.user_data.botoom_fix = 2
...
def handle_data(context):
    ...
    lpmin = np.max(hist2["high"].iloc[-context.user_data.cbwidth-1:-1])
    ...
    # 获取历史数据
    hist5 = context.data.get_price(context.security, count=10, frequency="15m")
    if len(hist5.index) < 10:
        context.log.warn("bar的数量不足, 等待下一根bar...")
        return
    ...
    if portfolio_rate < (context.user_data.portfolio_high - context.user_data.portfolio_cbrate) and portfolio_rate > context.user_data.portfolio_bottom:
        if ar < context.user_data.over_sell:
            ...
            if context.account.huobi_cny_cash/6 >= HUOBI_CNY_LTC_MIN_ORDER_CASH_AMOUNT:
                context.order.buy(context.security, cash_amount=str(context.account.huobi_cny_cash/6))
            return
        elif latest_close_price/lpmax < 0.9:
            setbprice(context, latest_close_price)
            context.order.buy(context.security, cash_amount=str(context.account.huobi_cny_cash/5))
            return
