...
def initialize(context):
    ...
    context.user_data.period_window = 16
    # 布林线的宽度（2倍标准差）
    ...
    # 空头ATR的倍数
    context.user_data.short_multi = 1.1
...
def handle_data(context):
    ...
    context.log.info("atr %s %s %s" % (atr, prev_price, lower))
    
    # 根据信号买入/卖出
    if short_signal_triggered or (current_price < lower and atr > 14):
